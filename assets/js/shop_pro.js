/**
 * Panier espace pro — tarifs serveur à la commande (pro_order.php).
 */
const BOOT = window.__TIRAMII_PRO__ || { csrf: '', products: [], account: null };
...
function resolveBox(boxKey) {
  const b = FIXED_BOXES[boxKey];
  if (!b) return null;
  ...
  return {
    ...
    stockId: b.stockId,
    name: b.name,
    price: p ? Number(p.price) : b.price,
    displayFlavors: b.displayFlavors,
    cartImgKey: b.cartImgKey,
  };
}

const SESSION_KEY = 'tiramii_pro_session_id';
const CART_KEY = 'tiramii_pro_cart';
const DELIVERY_TIME_LABEL = 'À convenir avec casadessert';
...
async function validateDeliveryForPro(_addr, zip, _city, _total) {
  const digits = String(zip).replace(/\D/g, '');
  if (digits.length < 5) {
    throw new Error('Indiquez un code postal valide (5 chiffres).');
  }
}
...
function updateHoldBanner() {
  ...
  banner.innerHTML = `<strong>⏳ Stock réservé ${formatCountdown(remaining)}</strong> Réservation 5 min.`;
}
  ...
  c.innerHTML = cart
    ...
    <div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        <div class="cart-item-price">${(item.price * item.qty).toFixed(2).replace('.', ',')}€ <span style="font-size:.72rem;opacity:.85">(tarif pro)</span></div>
...
window.placeOrder = async function () {
  ...
  try {
    await validateDeliveryForPro(addr, zip, city, cartTotal);
  } catch (e) {
    showToast(e.message || 'Adresse incomplète.');
    return;
  }
  ...
  try {
    const r = await fetch(apiUrl('pro_order.php'), {
      method: 'POST',
      credentials: 'same-origin',
      headers: {
        'Content-Type': 'application/json',
